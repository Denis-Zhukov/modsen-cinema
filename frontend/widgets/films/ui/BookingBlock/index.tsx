...
export const BookingBlock = forwardRef(
    ({ filmId }: Props, ref: ForwardedRef<HTMLDivElement>) => {
        ...
        const onlyDays = useMemo(
            () => daysAndMonths.map(({ day }) => day),
            [daysAndMonths],
        );
        ...
        const [book, { isSuccess, error }] = useBookMutation();
        ...
        const seatText = useMemo(() => {
            ...
            if (locale === 'ru') {
                return (
                    t('seats') + TextUtils.getEndingWordByPlural(seats.length)
                );
            }
            return t('seat');
        ...
        return (
            <StyledWrapper ref={ref} className={poppinsFont.className}>
                ...
                <StyledTitle className={nunitoSansFont.className}>
                    {t('title')}
                </StyledTitle>
                <DayPicker
                    days={onlyDays}
                    activeDay={activeIndexDay}
                    setActiveDay={setActiveIndexDay}
                />
                {isLoading ? (
                ...
                <StyledBooking>
                    <StyledSelectionDetails className={poppinsFont.className}>
                        <StyledSeatsInfo>
                            {seats.length} {seatText}
                        </StyledSeatsInfo>
                        <StyledPrice>
                            {Math.floor(price * seats.length * 100) / 100} $
                        </StyledPrice>
                    </StyledSelectionDetails>
